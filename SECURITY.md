...
`VITE_SUPABASE_URL` und `VITE_SUPABASE_ANON_KEY` sind für das Frontend zwingend erforderlich. Der Anon-Key ist ein öffentlicher Client-Key und kein Service-Role-Key. Der Service-Role-Schlüssel darf nie in `VITE_*`-Variablen oder im Browser stehen. Die echte Berechtigung erfolgt ausschließlich über Supabase Auth und `club_admin_memberships`.
