...
export const mailSubjects = {
  registration: 'Ihre Anmeldung zum Newcastle-Impftermin ist erfolgreich eingegangen',
  changed: 'Ihre Anmeldung wurde geändert',
  cancelled: 'Ihre Anmeldung wurde storniert',
  'reminder-7d': 'Noch 7 Tage bis zu Ihrem gebuchten Termin zur Newcastle-Impfung',
  'reminder-24h': 'Noch 24 Stunden bis zu Ihrem gebuchten Termin zur Newcastle-Impfung',
  'appointment-change': 'Wichtige Änderung zu Ihrem Newcastle-Impftermin',
  'new-appointment': 'Neuer Newcastle-Impftermin verfügbar'
}
...
export async function buildParticipantMail(kind, p, a, { test = false, clubSlug, changes = [] } = {}) {
  ...
  const appointment = `<p><strong>Newcastle-Impftermin:</strong> ${e(a.title || 'Newcastle-Impftermin')}<br><strong>Datum:</strong> ${e(formatDate(a.date))}<br><strong>Uhrzeit:</strong> ${e(appointmentTime(a) || 'Noch nicht angegeben')}${appointmentAddress(a) ? `<br><strong>Treffpunkt:</strong> ${e(appointmentAddress(a))}` : ''}</p>`
  const intro = {
    ...
    cancelled: 'Ihre Anmeldung wurde erfolgreich storniert. Für diesen Newcastle-Impftermin besteht keine aktive Anmeldung mehr.',
    'reminder-7d': 'Noch 7 Tage bis zu Ihrem gebuchten Termin zur Newcastle-Impfung.', 'reminder-24h': 'Noch 24 Stunden bis zu Ihrem gebuchten Termin zur Newcastle-Impfung.',
    'appointment-change': `Ihr Newcastle-Impftermin wurde geändert.${changes.length ? ` Geändert: ${changes.map(e).join(', ')}.` : ''} Bitte beachten Sie die aktuellen Angaben.`,
    'new-appointment': 'Ein neuer Newcastle-Impftermin steht zur Anmeldung zur Verfügung.'
  }[kind]
  ...
  if (kind === 'new-appointment') {
    ...
    html += `<p><a style="display:inline-block;padding:14px 20px;background:#123c2b;color:white;border-radius:6px" href="${e(url)}">Zum neuen Newcastle-Impftermin anmelden</a></p>`
  } else {
